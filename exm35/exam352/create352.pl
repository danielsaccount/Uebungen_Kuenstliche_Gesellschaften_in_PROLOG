% create352.pl (this module generates characters and data in SMASS)
...
make(wealth_dono) :- actors(AS), domain_of_wealth_dono(L,U),
   ...
   (   between(1,AS,A), nd_expr(wealth_dono,A,W), 
       append('data352.pl'),
...
make(wealth_take) :- actors(AS), domain_of_wealth_take(L,U),
    ...
    (  between(1,AS,A), nd_expr(wealth_take,A,W), 
       append('data352.pl'),
...
locate(A) :- cell_list(L), length(L,E), X is random(E)+1, nth1(X,L,Y),
  ...
  append('data352.pl'), write(fact(0,0,location(A,I,J))), write('.'), nl, told,
