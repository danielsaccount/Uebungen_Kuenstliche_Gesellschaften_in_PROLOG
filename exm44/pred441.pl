% pred441.pl (auxiliary predicates for SMASS)
...
determine_dd_value(N,A,EX,LIST) :- X is random(100)+1,
  between(1,EX,Z), nth1(Z,LIST,W_Z), X < W_Z, assert( dd_expression(N,A,Z)),!.
...
randomize(L,L1) :- 
   length(L,E), asserta(dyn_list_up([])),    asserta(dyn_list_down(L)),
   ( between(1,E,N), generate(N), fail; true),
   dyn_list_up(L1),
   retractall(dyn_list_up(LL)), retractall(dyn_list_down(LL1)).

generate(N) :- dyn_list_down(L2), dyn_list_up(L3), length(L2,E2),
   X is random(E2) + 1, nth1(X,L2,K), delete(L2,K,L2new),
   append(L3,[K],L3new),
   retract(dyn_list_down(L2)), asserta(dyn_list_down(L2new)),
   retract(dyn_list_up(L3)), asserta(dyn_list_up(L3new)),!.
